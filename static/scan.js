...
import { scanImageData } from "/static/zbar-wasm/index.mjs";
// Make available to scan.js
window.scanImageData = scanImageData;
...
async function handleCodeDetected(symbol) {
	try {
		...
		if (typeof symbol.decode === "function") {
			...
			console.error(
				"Cannot find decoded value in symbol:",
				Object.keys(symbol),
			);
		...
		console.log("Code detected and processed:", {
			typeName,
			codeType,
			codeValue,
		});
